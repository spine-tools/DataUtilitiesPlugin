...
class TestValidateFunction(unittest.TestCase):
    ...
    def _add_object_parameter(self, value):
        ...
        self.assertEqual(result, (1, []))
        result = import_object_parameter_values(self._db_map, (("o_class", "obj", "param", value),))
        self.assertEqual(result, (1, []))

    def _add_2D_relationship_and_parameter(self, value):
        result = import_object_classes(self._db_map, ("o_class", "p_class"))
        self.assertEqual(result, (2, []))
        result = import_objects(self._db_map, (("o_class", "o_obj"), ("p_class", "p_obj")))
        self.assertEqual(result, (2, []))
        result = import_relationship_classes(self._db_map, (("r_ship", ("o_class", "p_class")),))
        self.assertEqual(result, (1, []))
        ...
        self.assertEqual(result, (1, []))
        result = import_relationship_parameter_values(self._db_map, (("r_ship", ("o_obj", "p_obj"), "param", value),))
        self.assertEqual(result, (1, []))

    def test_empty_database_and_empty_settings(self):
        settings = {}
        errors = validate(self._db_map, settings)
    ...
    def test_single_relationship_value_fails_type_check(self):
        ...
        errors = validate(self._db_map, settings)
        self.assertEqual(errors, ["r_ship - param - o_obj,p_obj - Base: must be of integer type"])

    def test_additional_type_rules_fail_correctly(self):
        self._add_object_parameter(23.0)
        for type_ in ("array", "datetime", "duration", "map", "time pattern", "time series"):
            settings = {
                "object_parameter_value": [
                    {
                        "rule": {"type": type_}
            ...
            errors = validate(self._db_map, settings)
            self.assertEqual(errors, [f"o_class - param - obj - Base: must be of {type_} type"])
    ...
    def test_number_of_indexes_rule(self):
        ...
        for type_ in ("array", "datetime", "duration", "map", "time pattern", "time series"):
            settings = {
                "object_parameter_value": [
                    {
                        "rule": {"number of indexes": 2}
                    }
                ]
            }
            errors = validate(self._db_map, settings)
            self.assertEqual(errors, [f"o_class - param - obj - Base: must have index count of 2"])
